...
### Added
- **Sea level tool (`type: "sealevel"`)** — a slider from the lowest sea level of the last glacial maximum (−134 m, ~21 ka) to +70 m, with step buttons, playback and a jump back to today. It drives one layer whose polygons carry the sea level at which they connect to the ocean (`flood_level`; `layer: "coastal-zones"` by default): water at or below the level, dry sea floor between it and today, everything else transparent so the basemap shows today's land and lakes. On opening, the layer is split once (`setSubLayers`) into one sublayer per class with a constant colour, and a move rewrites only the classes that cross the level — one per metre of playback. A single data-driven `fill-color` expression was re-evaluated for every feature of every loaded tile on each move; measured, a 1 m step went from ~335 ms to ~32 ms above a plain repaint. Each class is retitled by its role as it flips, so the legend reads "Sea", "Dry sea floor" and "As today" at whatever level the slider stands. Class levels default to the coastal zones ETL's (−134 m, multiples of 5 m, and 1 m from −1 to +10 m) and can be set with `levels`. With a `data` curve — a config asset of `[age ka BP, metres]` points, `utils/sea-level-curve.ts` — the tool also has a **time mode**: the slider runs through years, sets the level the curve gives for each age, names the period (last glacial maximum, meltwater pulse 1A, Younger Dryas, the Black Sea connecting, Doggerland drowning) and plays forward at 250–2,000 years per second. The demo config ships an approximation of Lambeck et al. (2014) for the last 26,000 years.
- **`pmtiles://` vector sources (MapLibre).** A vector source's `url` may name a single `.pmtiles` archive, read with HTTP range requests, so tiles need no tile server — any static host serves them. The protocol is registered once in the MapLibre adapter. A relative archive path (`pmtiles://../data/zones.pmtiles`) is resolved against the config like every other path; without that its scheme left it relative to the page.
- **Plugins add tools that behave like built-in ones.** A config's `plugins` lists module URLs — relative to the config, or on a trusted CDN — and each is loaded before the config is validated, so its tools can be named in `tools` like any other: toolbar button, panel, validator, and a place in `testpages/setup.html`, which can also add a plugin by URL and edit its settings with ⚙. A plugin calls `registerTool()` with the classes handed to its `register(api)`, and can register a `configTemplate` as its default settings; it cannot take a built-in tool's id or element. `public/plugins/bookmarks.js` is a working example (named map views). See `docs/developer/plugin-authoring.md`.
- **`webmapx-map.getLayerDefinition(layerId)`** returns a layer config-shaped, as it now stands: the definition it was added with, carrying whatever the user has since changed about it. A style edit reaches the engine and `store.mapLayers` and never the config the layer came from, so anything handing a layer on — a "copy config" button, a config editor, an export — cannot read the config and be right. Doing it by hand is the same merge every time (a composite keeps its paint per sublayer, a style edit may have *added* a label layer, a rename lives on the store entry), which is why it is one method rather than each caller's own guess at it. `visible` and `transparency` come back beside the layer rather than inside it: they are map state, which the permalink already carries, not part of what the layer is. Paint is deliberately still absent from the permalink — a data-driven `match` with dozens of stops belongs in a config file, not in a URL.
...
### Fixed
- **The map can be reached and used from the keyboard on every engine.** It is one landmark named after the config's `map.label`, the first Tab lands on it, and arrow keys pan and `+`/`-` zoom. MapLibre and Leaflet already did this; OpenLayers could not be focused and Cesium had no keyboard control at all. Where an engine handles keys itself it keeps doing so (`NavigationCapabilities.keyboard`), so a key never moves the map twice. The map region is `aria-busy` while loading, and the spinner no longer presents itself as a permanent "Loading" progress bar.
- **Tool results are announced to screen readers.** Search, info (on click), measure, geoprocessing and routing say what they found through one live region per map ("3 results for Utrecht", "Distance 12.4 km", errors included), instead of changing their panel silently.
- **Draw modes and snap say whether they are on** (`aria-pressed`), and the inset map is a single "Overview map" image rather than a second, identically named map landmark.
- **Contrast and headings:** layer names in the legend no longer turn a 4.09:1 blue on hover, tool panel titles are `h2` (headings inside no longer skip a level), and the settings panel's section headings follow suit. `scripts/ui-tests/a11y.mjs` runs axe-core with every panel open on all four engines, plus the keyboard, announcement and toggle checks axe cannot make.
- **A failed print no longer leaves its hidden page behind.** When the map could not be rendered for printing, the off-screen print page stayed in the document.
- **A toolbar item that names only its `type` opens its tool.** Without an `id` the button had no name and the tool was registered as `"undefined"`, so clicking it did nothing. The type now doubles as the id, as it already did inside a toolbox or menu.
- **Icon-only buttons have an accessible name.** The draw toolbar, the draw-layer property editor and the legend's clickable colour swatch relied on a tooltip, which screen readers do not announce as the button's name.
- **Search results add and remove their own layer.** A result row's checkbox is now a layer icon whose badge is a plus, and a green check once the result is on the map — the same button removes it again, so undoing an add never means hunting for the layer in the legend. State is carried by shape as well as colour, the button keeps one accessible name with `aria-pressed` for its state, and adding a layer deliberately does *not* open the legend: every other way of adding a layer leaves it closed, tool panels are exclusive, and on a phone the legend would replace the search results the user is still working through. The field has a × to empty the query and its results, which is what deactivating the tool used to do silently — the panel now keeps its state when you switch tools, so emptying it is an action you take rather than one that happens to you. Persistence is keyed by the result's own identity (its OSM or place id, else its title and first coordinate) rather than by the feature object, so searching the same place twice still shows it as added instead of offering a plus for a layer that is already there.
...
- **A layer with no paint gets webmapx's unstyled look, not one guess per engine.** A layer definition may carry no `paint` at all — which is exactly what a catalog that only knows the service produces, such as a harvested WFS layer (`{ type: 'fill', source, metadata }` and nothing more). Each engine then answered with its own default: MapLibre follows the style spec (`fill-color` `#000000`, opaque), while this codebase's Leaflet and Cesium factories fell back to `#3388ff`, as did the legend. So the same layer came out as an opaque black blob covering the map while its legend swatch showed blue, and two halves of one screen disagreed about its colour. `withDefaultPaint` (`src/map/default-paint.ts`) now fills the paint in once, in `BaseAdapter.addLayer`, before any engine sees it — the legend follows the layer rather than inventing a colour of its own. The values are the ones a dropped file with no style already got: a dark outline over a translucent grey fill (`#444444` at 0.3), so a layer whose style was never authored, forgotten, or lost to a bug looks the same however it arrived. Every remaining per-property fallback in the engines and the legend reads the same constants, so partial paint cannot reintroduce the split. Only a wholly absent paint is filled in, and no sub-layer is ever added: a layer that sets one property and leaves the rest to the spec means that.
...
### Changed
- **Generic code asks the adapter what an engine can do instead of checking its name.** New `IMap` methods: `canDrawSource`, `canDrawLayerType`, `getViewProjections`, `getTerrainSourceKind`, and optional `renderPrintMap`. `webmapx-map`, the print, projection and 3D tools no longer branch on `engineId`, and the print tool no longer imports `maplibre-gl` or reaches into the adapter. An engine added later declares its own support. The WMS URL builder always emits the style-spec template (`{bbox-epsg-3857}`); its `engine` parameter only ever received `'maplibre'`.
...
### Removed
- **`LayerSpec`, `FillPaint` and `LinePaint`**, a narrow layer dialect used only for sub-maps. `ISubMap.createLayer` now takes `SubMapLayerSpec`, a style-spec fill/line/circle/symbol layer (`source` instead of `sourceId`).
