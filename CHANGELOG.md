...
### Fixed
- **Search results add and remove their own layer.** A result row's checkbox is now a layer icon whose badge is a plus, and a green check once the result is on the map — the same button removes it again, so undoing an add never means hunting for the layer in the legend. State is carried by shape as well as colour, the button keeps one accessible name with `aria-pressed` for its state, and adding a layer deliberately does *not* open the legend: every other way of adding a layer leaves it closed, tool panels are exclusive, and on a phone the legend would replace the search results the user is still working through. Persistence is keyed by the result's own identity (its OSM or place id, else its title and first coordinate) rather than by the feature object, so searching the same place twice still shows it as added instead of offering a plus for a layer that is already there.
