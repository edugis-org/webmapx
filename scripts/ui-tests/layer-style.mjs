...
async function openStylePanel(page) {
    await page.evaluate(async ([layerId]) => {
        ...
        const source = adapter.getSourceData(`${layerId}-src`);
        dialog.open({
            title: 'Style test',
            layerId,
            groups: [{
                ...
                attributes: [
                    { name: 'name', type: 'string', values: source.features.map(f => f.properties.name), presentCount: 9, missingCount: 0 },
                    { name: 'pop', type: 'number', values: source.features.map(f => f.properties.pop), presentCount: 9, missingCount: 0 },
                    { name: 'region', type: 'string', values: source.features.map(f => f.properties.region), presentCount: 9, missingCount: 0 },
                ],
                featureRows: source.features.map(f => f.properties),
                layers: [{ id: layerId, type: 'fill', paint: { 'fill-color': '#888888', 'fill-opacity': 0.6 } }],
