...
- Decisions: None.
- Next steps:
	1. Move the central store from global scope into each `<gis-map>` instance (one store per map) so multiple maps can coexist without shared state collisions.
	2. Allow tools to live outside `<gis-map>` but reference the intended map via an attribute (e.g., `map-target="map-container"`); automatically resolve the host map when a tool is slotted inside `<gis-map>` so authors only set the attribute for out-of-map tools.
