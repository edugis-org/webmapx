...
| ~~Keyed neighbour colouring for tiled sources~~ (`buildKeyedColorStyle`, `coloringKeyFor`) | "Neighbours differ" on a source it could not write into, as a `match` on a unique column | **Ported.** A source the app cannot write into gets `By neighbours` whenever `coloringKeyFor` finds a key (the feature id, one unique column, or columns unique together). `NeighbourChannel` carries `key` + `assignments` and encodes a `match` on `coloringKeyExpression`; a single column is written as a one-argument `concat`, so a reopened panel decodes it as neighbours rather than as categories of that column. The areas coloured are the ones the map has drawn (the sampler reassembles tile-split pieces, so each area has one key); one that arrives later takes the fallback colour until the colouring is run again, which the panel says. Tested round-trip for all three key kinds through MapLibre's own evaluator |
| ~~Palette choice for a neighbour colouring~~ (`renderSchemeStep` in neighbours mode) | Scheme list, Reverse and Colour-blind safe for "Neighbours differ" | **Ported** after it was missed: the styler first fixed a neighbour colouring to the first qualitative scheme. The same palette row `By attribute` shows now sits under the Colours slider, qualitative only, held per channel apart from the classification settings (routing it through `updateSettings` would repaint the channel as a classification) and re-running the colouring on change. With Colour-blind safe ticked above four colours no scheme qualifies (ColorBrewer rates Set2, Dark2 and Paired safe at three colours, only Paired at four, none above). The warning is shown in the palette row, judged on the count the slider asks for, and says the map still shows the last colouring; it first went to the top of the panel, out of view, while the row went on showing palettes for the colouring still on the map. Okabe-Ito (8, black last so seven or fewer never contain it), Paul Tol bright (7) and muted (9) are added as qualitative palettes (`src/utils/color-schemes-cvd.ts`), rated colour-blind and screen `ok` and print/photocopy `unknown`, after ColorBrewer so lists still open on the same scheme. Colour-blind-safe qualitative palettes now reach nine colours (4: Paired + the three; 5-7: the three; 8: Okabe-Ito and Tol muted; 9: Tol muted), and the most palettes on offer at once rises from 8 to 11 |
