...
- **Legend "other" suppression trick:** `webmapx-layer-legend.ts` renders a legend row per `match` expression case, including a trailing `'other'` row for the fallback. To suppress it, use an **array expression** as the fallback instead of a plain string — e.g. `['literal', 'rgba(0,0,0,0)']`. The legend checks `typeof fallback === 'string'` (in `extractColorClasses`) and `Array.isArray(fallback)` (in `extractDataCases`) and skips the "other" row when the fallback is an array. MapLibre evaluates the expression normally on the map.
